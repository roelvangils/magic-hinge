...
p.add_argument('--motion',choices=['scroll','idle'],default='scroll')
a=p.parse_args()
...
kind='front' if a.motion=='scroll' else 'idle'
folder=f'air-{kind}-{a.appearance}-{hash.hexdigest()[:12]}'
...
if a.motion=='idle': attribute='data-idle-sequence' if a.appearance=='light' else 'data-dark-idle-sequence'
